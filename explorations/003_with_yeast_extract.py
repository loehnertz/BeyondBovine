...
# ///
"""EXPLORATORY: 002 plus yeast extract as a second carbon source.

Step 5 of the first goal (see docs/LEARNING.md). This is a learning model, not the engine.
Identical to 002_overflow_repression.py except for yeast extract (YE).
...
Run with:  uv run explorations/003_with_yeast_extract.py
"""
...
def calibrate(data: dict[str, np.ndarray], kr: float, repression: bool, yeast_extract: bool) -> Params:
    ...
    def make(v) -> Params:
        return Params(*v[:5], kr=kr, repression=repression, yeast_extract=yeast_extract,
                      y_xye=v[5] if yeast_extract else 0.0)
    ...
    #         qs_max qcrit_min k_rec qe_max y_xe [y_xye]
    start = [2.0, 0.2, 1.0, 0.3, 0.4] + ([0.3] if yeast_extract else [])
    lower = [0.2, 0.0, 0.01, 0.01, 0.05] + ([0.0] if yeast_extract else [])
    upper = [10.0, QCRIT_MAX, 20.0, 3.0, 0.8] + ([1.0] if yeast_extract else [])
...
def main() -> None:
    ...
    models["repression, no YE (002)"] = calibrate(data, KR_VALUES[1], True, False)

    print("Calibrated on 40 g/L only:")
    for name, p in models.items():
        print(f"  {name:24s} qs_max={p.qs_max:.2f} qcrit_min={p.qcrit_min:.2f} "
              f"k_rec={p.k_rec:.2f} qe_max={p.qe_max:.2f} y_xe={p.y_xe:.2f} y_xye={p.y_xye:.2f}")
...
def plot(data, models, table, levels) -> None:
    ...
    colors = {main_name: "#2a78d6", "constant+YE": "#eb6834", "repression, no YE (002)": "#a3a29c"}
    ...
    fig.suptitle("Overflow + glucose repression + yeast extract: exploratory, calibrated on 40 g/L only",
                 color=ink, fontsize=11)
    fig.tight_layout()
    fig.savefig(HERE / "003_with_yeast_extract.png", dpi=140)
