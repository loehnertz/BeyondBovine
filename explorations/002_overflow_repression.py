...
# ///
"""EXPLORATORY: batch yeast with overflow to ethanol and glucose repression.
...
Run with:  uv run explorations/002_overflow_repression.py
"""
...
KS = 0.1  # g/L, sugar level at which uptake is half its maximum (Verduyn 1990, ~0.55 mM)
KE = 0.1  # g/L, the same for ethanol. ASSUMPTION: order of magnitude, borrowed from KS.
...
def rates(p: Params, x: float, s: float, e: float, r: float) -> tuple[float, float, float, float]:
    ...
    mu = Y_OX * q_resp + Y_RED * q_over + p.y_xe * qe
    dx = mu * x
    ds = -qs * x
...
def simulate(p: Params, s0: float, x0: float, t_end: float, t_eval=None):
    sol = solve_ivp(
        lambda t, y: rates(p, *y), (0, t_end), [x0, s0, 0.0, R0],
...
def glucose_phase_yields(p: Params, s0: float, x0: float) -> tuple[float, float]:
    ...
    t, (x, s, e, _) = simulate(p, s0, x0, t_end=40.0)
...
def calibrate(data: dict[str, np.ndarray], kr: float, repression: bool) -> Params:
    t = data["t_h"]

    def residuals(v):
        p = Params(*v, kr=kr, repression=repression)
        _, (x, s, e, _) = simulate(p, 40.0, data["cells_g_per_L"][0], t[-1], t_eval=t)
        if len(x) != len(t):
    ...
    #         qs_max qcrit_min k_rec qe_max y_xe
    start = [2.0, 0.2, 1.0, 0.3, 0.4]
    lower = [0.2, 0.0, 0.01, 0.01, 0.05]
    upper = [10.0, QCRIT_MAX, 20.0, 3.0, 0.8]
    fit = least_squares(residuals, start, bounds=(lower, upper))
    return Params(*fit.x, kr=kr, repression=repression)


def main() -> None:
    data = load_fig1()
    x0 = data["cells_g_per_L"][0]  # same 5 % inoculum in every run
    models = {f"repression, KR={kr} g/L": calibrate(data, kr, True) for kr in KR_VALUES}
    models["constant threshold"] = calibrate(data, KR_VALUES[1], False)

    print("Calibrated on 40 g/L only:")
    for name, p in models.items():
        print(f"  {name:24s} qs_max={p.qs_max:.2f} qcrit_min={p.qcrit_min:.2f} "
              f"k_rec={p.k_rec:.2f} qe_max={p.qe_max:.2f} y_xe={p.y_xe:.2f}")
...
def plot(data, models, table, levels) -> None:
    ...
    main_name = "repression, KR=0.5 g/L"
    colors = {main_name: "#2a78d6", "constant threshold": "#eb6834"}
    ...
    for ax, j, title in ((axes[1, 0], 0, "Test: cell yield, glucose phase (g/g)"),
        ...
        kr_names = [n for n in models if n.startswith("repression")]
    ...
    fig.suptitle("Overflow + glucose repression: exploratory model, calibrated on 40 g/L only",
                 color=ink, fontsize=11)
    fig.tight_layout()
    fig.savefig(HERE / "002_overflow_repression.png", dpi=140)
