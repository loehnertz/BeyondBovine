...
# ///
"""EXPLORATORY: 005 with two separate adaptation states: transporters (fast) and respiration (slow).
...
Change in 006 (agreed with Jakob): 005 could not overflow at low sugar, because one state R set both
fast uptake (high affinity) and a high respiration threshold. HYPOTHESIS: transporter affinity T adapts
to the glucose level faster than respiration R derepresses. So:

    KS(T) = KS_HIGH_AFF * T + ks_low_aff * (1 - T)       T: 0 = low-affinity, 1 = high-affinity
    dT/dt = k_t * ((1 - g) - T),  with k_t = k_rec * speed_ratio, speed_ratio >= 1 (calibrated)
    R as before (respiration threshold, ethanol use)

The ordering "transporters faster than respiration" is the hypothesis. It is built in through
speed_ratio >= 1 and is NOT sourced quantitatively. Cells start with T0 = 0 and R0 = 0.
...
Run with:  uv run explorations/006_two_states.py
"""
...
def rates(p: Params, x, s, e, r, ye, tr):
    s, e, ye = max(s, 0.0), max(e, 0.0), max(ye, 0.0)
    g = s / (s + KR)
    ks = KS_HIGH_AFF * tr + p.ks_low_aff * (1 - tr)
    qs = p.qs_max * s / (ks + s)
    ...
    return (
        ...
        -q_ye * x - d * ye,
        p.k_rec * p.speed_ratio * ((1 - g) - tr),
    ), (qs, Y_ETOH * q_over - qe)


def simulate_batch(p: Params, s0: float, x0: float, t_end: float, t_eval=None, r0=0.0, with_ye=True, t0=None):
    ye0 = YE0.get(int(s0), 0.0) if with_ye else 0.0
    p = replace(p, ye_ratio=ye0 / s0, dilution=0.0)
    t0 = r0 if t0 is None else t0
    sol = solve_ivp(lambda t, y: rates(p, *y)[0], (0, t_end), [x0, s0, 0.0, r0, ye0, t0],
...
def chemostat_steady_state(p: Params, d: float) -> tuple[float, float, float]:
    ...
    y0 = [Y_OX * VANHOEK_FEED * 0.9, 0.05, 0.0, 1.0, 0.0, 1.0]
    sol = solve_ivp(lambda t, y: rates(p, *y)[0], (0, 150), y0, method="LSODA", rtol=1e-7, atol=1e-9)
    x, s, e, r, ye, tr = sol.y[:, -1]
    (_, (qs, qe_net)) = rates(p, x, s, e, r, ye, tr)
    return qs, qe_net, x
...
def calibrate(ji: dict, vh: dict) -> Params:
    ...
    #         qs_max qcrit_min qcrit_max k_rec qe_max y_xe ks_low y_xye speed_ratio
    start = [4.0, 0.3, 0.75, 2.0, 0.16, 0.33, 10.0, 0.05, 5.0]
    lower = [0.5, 0.0, 0.2, 0.05, 0.01, 0.05, 0.3, 0.0, 1.0]
    upper = [10.0, 1.5, 1.5, 20.0, 3.0, 0.8, 30.0, 1.0, 100.0]
...
def plot(p, ji, chem, levels, table, hb, hb_runs) -> None:
    ...
    fig.suptitle("Two states (fast transporters, slow respiration): exploratory, calibrated on Ji 40 g/L + van Hoek chemostat",
                 color=ink, fontsize=11)
    fig.tight_layout()
    fig.savefig(HERE / "006_two_states.png", dpi=140)
